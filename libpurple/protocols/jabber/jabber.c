...
static void jabber_toggle_carbons(PurplePluginAction *action) {
	PurpleConnection *gc = (PurpleConnection *) action->context;
	JabberStream *js = purple_connection_get_protocol_data(gc);
	JabberIq *iq = jabber_iq_new(js, JABBER_IQ_SET);
...
static void
jabber_mam_prefs_change_result_cb(JabberStream *js, const char *from,
                                 JabberIqType type, const char *id,
                                 xmlnode *packet, gpointer data)
{
	if (type == JABBER_IQ_RESULT) {
		purple_notify_info(js->gc, _("MAM Preferences Changed"), _("MAM Preferences Changed"),
				_("Your server-side MAM preferences have been changed."));
	} else {
		char *msg = jabber_parse_error(js, packet, NULL);

		purple_notify_error(js->gc, _("Error changing MAM preferences"),
				_("Error changing MAM preferences"), msg);
		g_free(msg);
	}
...
static void jabber_mam_prefs_change_cb(JabberStream *js,
		PurpleRequestFields *fields)
{
	JabberIq *iq;
	xmlnode *prefs, *node, *valuenode;
	char **pieces, **p;
	int pref_result = purple_request_fields_get_choice(fields, "mam_pref");
	char *mam_default = "never";
	...
	pieces = g_strsplit(mam_always, "\n", -1);
	if (pieces) {
		for (p = pieces; *p != NULL && !purple_strequal(*p, ""); p++) {
			if (strlen(*p) <= 0)
				continue;

			valuenode = xmlnode_new_child(node, "jid");
			xmlnode_insert_data(valuenode, *p, -1);
		}
	}

	node = xmlnode_new_child(prefs, "never");
	pieces = g_strsplit(mam_never, "\n", -1);
	if (pieces) {
	...
	jabber_iq_set_callback(iq, jabber_mam_prefs_change_result_cb, NULL);

	jabber_iq_send(iq);
}

static void jabber_mam_prefs(JabberStream *js, const char *from,
                                 JabberIqType type, const char *id,
                                 xmlnode *packet, gpointer data)
{
	PurpleRequestFields *fields;
	PurpleRequestFieldGroup *group;
	PurpleRequestField *field;
	int pref_result = MAM_PREFS_NEVER;
	xmlnode *node, *jidnode, *valuenode;
	...
	if (type != JABBER_IQ_RESULT || !node) {
		...
		purple_notify_error(js->gc, _("Error requesting MAM preferences"),
				_("Error requesting MAM preferences"), msg);
		g_free(msg);

		return;
	}

	mam_default = (char *) xmlnode_get_attrib(node, "default");
	if (purple_strequal(mam_default, "always"))
		pref_result = MAM_PREFS_ALWAYS;
	else if (purple_strequal(mam_default, "roster"))
		pref_result = MAM_PREFS_ROSTER;

	purple_debug_info("jabber", "MAM Preferences received.\n");

	fields = purple_request_fields_new();
	group = purple_request_field_group_new(NULL);
	purple_request_fields_add_group(fields, group);

	field = purple_request_field_choice_new("mam_pref", _("Default archive setting"), pref_result);
	purple_request_field_choice_add(field, _("Never"));
	purple_request_field_choice_add(field, _("Roster"));
	purple_request_field_choice_add(field, _("Always"));
	purple_request_field_set_required(field, TRUE);
	purple_request_field_group_add_field(group, field);

	str = g_string_new("");
	jidnode = xmlnode_get_child(node, "always");
	for(valuenode = xmlnode_get_child(jidnode, "jid"); valuenode;
	...
	field = purple_request_field_string_new("mam_always", _("Always archive messages for these Jabber IDs"), str->str, TRUE);
	g_string_free(str, TRUE);
	purple_request_field_group_add_field(group, field);

	str = g_string_new("");
	jidnode = xmlnode_get_child(node, "never");
	for(valuenode = xmlnode_get_child(jidnode, "jid"); valuenode;
		valuenode = xmlnode_get_next_twin(valuenode)) {
	...
	field = purple_request_field_string_new("mam_never", _("Never archive messages for these Jabber IDs"), str->str, TRUE);
	g_string_free(str, TRUE);
	purple_request_field_group_add_field(group, field);

	purple_request_fields(js->gc, _("Server-side Archiving Preferences"),
			_("Server-side Archiving Preferences"), _("Please select your preferred settings"),
			fields, _("OK"), G_CALLBACK(jabber_mam_prefs_change_cb),
			_("Cancel"), NULL,
			purple_connection_get_account(js->gc), NULL, NULL,
			js);
...
static void jabber_mam(PurplePluginAction *action) {
	PurpleConnection *gc = (PurpleConnection *) action->context;
	JabberStream *js = purple_connection_get_protocol_data(gc);
...
GList *jabber_actions(PurplePlugin *plugin, gpointer context)
{
	...
	if(js->server_caps & JABBER_CAP_MAM) {
		act = purple_plugin_action_new(_("Messaging Archive Management"), jabber_mam);
		m = g_list_append(m, act);
	}
